...
- **修复 EXE 不生成配置文件** — 打包后 `__file__` 指向临时目录，配置文件写入后随临时目录一同删除。改用 `sys.executable` 定位 EXE 所在目录，配置文件和日志目录现在正确生成在 EXE 旁边

### 文档

- **README.md 同步更新** — 项目结构新增 `fonts/` 目录，功能特性补充字体优化及吸附指示器说明，技术实现新增吸附机制章节
- **配置说明通俗化** — 配置文件参数描述重写为通俗语言，附使用场景说明
- **发布 v1.4 EXE 安装包** — 使用 PyInstaller 打包为单文件 EXE，内置字体和资源文件
